...
public class NetRequest {
    ...
    public static void postFiles(String url, Map<String, String> params,List<String> keys, List<File> files, DataCallBack callBack){
        getInstance().inner_postImagesAsync(url, params, keys, files, callBack);
    }
    //-------------对外提供的方法End--------------------------------
    ...
    private void inner_postImagesAsync(String url, Map<String, String> params, List<String> filekeys, List<File> files, final DataCallBack callBack) {
        RequestBody requestBody;
        if (params == null) {
            params = new HashMap<>();
        }
        ...
        if (files!=null){
            for (File file:files){
                builder.addFormDataPart(filekeys.get(files.indexOf(file)), file.getName(),
                        RequestBody.create(MediaType.parse("image/png"), file));
            }
        }
        requestBody = builder.build();
